...
+!start : true <- .print("Communication specialist agent enabled.").//; !responder("operator","respId","Allocate Patient",[param("paciente","2044429"),param("leito","3132")],[]).
...
<-	!getInContext(Contexts, "leito", Bed);
	...
	!getInContext(Contexts, "paciente", Patient);
	.print("Chatbot of ", RequestedBy, " is requesting to verify suitability: paciente: ", Patient, ", leito: ", B);
	.send(assistant,assert,adequado(B,Patient));
	.
...
+!kqml_received(Sender,question,answer(_,Response),MsgId)
	<-	.print("Asking to chatbot: ", Response);
		reply(Response).
		
//+!kqml_received(Sender,question,answer2(_,Response),MsgId)
//	<-	.print("Answering to chatbot: ", Response);
//		reply(Response).
