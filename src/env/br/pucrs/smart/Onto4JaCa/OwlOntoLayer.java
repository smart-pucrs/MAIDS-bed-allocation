package br.pucrs.smart.Onto4JaCa;
...
import br.pucrs.smart.Onto4JaCa.mas.AxiomTranslator;
...
public class OwlOntoLayer {
   ...
   public List<OWLDataProperty> getDataProperties() {
    ...
    public List<OWLDataPropertyAssertionAxiom> getDataPropertyAssertionAxioms() {
    	List<OWLIndividual> individuals = getIndividuals();
    	List<OWLDataPropertyAssertionAxiom> axioms = new ArrayList<OWLDataPropertyAssertionAxiom>();
    	individuals.forEach(i -> {
    		axioms.addAll(this.ontology.dataPropertyAssertionAxioms(i).collect(Collectors.toList()));
    	});
    	return axioms;
    }
